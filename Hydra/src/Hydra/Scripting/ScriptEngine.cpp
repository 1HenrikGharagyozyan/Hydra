...
// ECMA-335 field-access flag (System.Reflection.FieldAttributes.Public). This
// Mono version's dev package doesn't ship mono/metadata/tabledefs.h (where
// other Mono versions define it), but the value itself is a fixed part of the
// CLI metadata spec and doesn't vary across Mono/.NET versions or platforms.
...
namespace Hydra
{
	...
	static std::unordered_map<std::string, ScriptFieldType> s_ScriptFieldTypeMap =
	{
		...
		{ "Hydra.Vector2",  ScriptFieldType::Vector2 },
		{ "Hydra.Vector3",  ScriptFieldType::Vector3 },
		{ "Hydra.Vector4",  ScriptFieldType::Vector4 },

		{ "Hydra.Entity",   ScriptFieldType::Entity },
	};
	...
	namespace Utils 
	{
		...
		ScriptFieldType MonoTypeToScriptFieldType(MonoType* monoType)
		{
			...
			{
				HD_CORE_ERROR("Unknown type: {}", typeName);
	...
	void ScriptEngine::LoadAssemblyClasses()
	{
		...
		for (int32_t i = 0; i < numTypes; i++)
		{
			...
			HD_CORE_WARN("{} has {} fields:", className, fieldCount);
			...
			while (MonoClassField* field = mono_class_get_fields(monoClass, &iterator))
			{
				...
				if (flags & FIELD_ATTRIBUTE_PUBLIC)
				{
					...
					HD_CORE_WARN("  {} ({})", fieldName, Utils::ScriptFieldTypeToString(fieldType));
