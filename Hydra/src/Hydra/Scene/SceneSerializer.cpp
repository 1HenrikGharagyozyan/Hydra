...
#define READ_SCRIPT_FIELD(FieldType, Type)             \
	...
	static std::string RigidBody2DBodyTypeToString(Rigidbody2DComponent::BodyType bodyType)
	{
		...
		HD_CORE_ASSERT(false, "Unknown body type");
		return {};
	}
	...
	{
		HD_CORE_ASSERT(entity.HasComponent<IDComponent>());
	...
	void SceneSerializer::Serialize(const std::string& filepath)
	{
		...
		if (!fout.is_open())
		{
			HD_CORE_ERROR("Could not open file '{0}' for serialization. Make sure the directory exists!", filepath);
			return;
		}
	...
	void SceneSerializer::DeserializeEntities(YAML::Node& data)
	{
		...
		if (entities)
		{
			for (auto entity : entities)
			{
				...
				HD_CORE_TRACE("Deserialized entity with ID = {0}, name = {1}", uuid, name);
